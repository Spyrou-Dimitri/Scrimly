...
new class extends Component
{
    ...
    public function startScrim(): void
    {
        abort_unless(
            $this->scrim->team_id === currentTeam()->id,
            403,
        ...
        $this->dispatch('toast', [
            'title' => __('modals/scrims/start-scrim.success_title'),
            'message' => __('modals/scrims/start-scrim.success_message'),
            'type' => 'check',
        ]);
    }
};
...
<div class="w-full">
    <x-layout.head-modal
        :width="'2xl'"
        :title="__('modals/scrims/start-scrim.title') . ' • ' . ($this->scrim->opponentTeam?->name ?? __('pages/scrims/show.opponent_unknown'))"
    >
        <form wire:submit.prevent="startScrim" class="flex w-full flex-col gap-6 pt-2">
            <div class="flex w-full flex-col gap-3">
                <div
                    class="mx-auto flex size-14 shrink-0 items-center justify-center rounded-none bg-gold/10 ring-1 ring-gold/40"
                    aria-hidden="true">
                    <flux:icon name="play" class="size-8 text-gold" />
                </div>

                <p class="text-center text-2xl font-bold text-text-primary">
                    {{ __('modals/scrims/start-scrim.body_heading') }}
                </p>
                <p class="text-center text-base font-normal text-text-secondary">
                    {{ __('modals/scrims/start-scrim.body_legend') }}
                </p>
            </div>
            ...
            <div class="flex w-full flex-wrap justify-center gap-3 sm:justify-between">
                <button
                    ...
                    type="button"
                    title="{{ __('modals/scrims/start-scrim.cancel') }}"
                    class="cta-secondary">
                    {{ __('modals/scrims/start-scrim.cancel') }}
                </button>
                <button
                    type="submit"
                    title="{{ __('modals/scrims/start-scrim.confirm') }}"
                    class="cta-primary cursor-pointer">
                    {{ __('modals/scrims/start-scrim.confirm') }}
                </button>
            </div>
        </form>
    </x-layout.head-modal>
